...
// API Fetch avec gestion automatique du token
  ...
  const login = async (username, password) => {
    try {
      ...
      if (data.success && data.token) {
        // Stocker le token et calculer l'expiration (7 jours)
...
// ============================================================================
// UI COMPONENTS - Design Professionnel
// ============================================================================
          ...
          {serviceStatus?.isOnDuty ? (
            <div className="flex items-center gap-2 px-3 py-2 bg-emerald-900/20 border border-emerald-800 rounded text-xs">
              ...
              <span className="text-emerald-600 ml-auto font-mono">
                {formatDuration(serviceStatus.service?.current_duration)}
              </span>
            </div>
          ) : (
        ...
        <div className="p-3 bg-slate-950 border-t border-slate-800">
          ...
          >
            ...
            <div className="overflow-hidden flex-1">
              ...
              <p className="text-xs text-slate-500 truncate">{user?.grade_name}</p>
            </div>
          </button>
          <button 
            onClick={logout} 
    ...
    <Layout>
      ...
      <div className="mb-8">
        ...
        <p className="text-slate-500 text-sm mt-1">
          {user?.grade_name} • Matricule {user?.badge_number || "N/A"}
        </p>
      </div>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <StatCard 
          ...
          value={myStats?.my_appointments || 0} 
          icon={ClipboardList} 
          color="blue" 
        />
        <StatCard 
          label="Temps de patrouille" 
          value={formatDuration(user?.total_patrol_time || 0)} 
          icon={Timer} 
          color="green" 
        />
        <StatCard 
          label="Statut" 
          value={serviceStatus?.isOnDuty ? "En service" : "Hors service"} 
          icon={Radio} 
          color={serviceStatus?.isOnDuty ? "green" : "yellow"} 
        />
        <StatCard 
          label="Patrouille" 
          value={serviceStatus?.patrol?.name || "Aucune"} 
          icon={Car} 
          color="purple" 
        />
      </div>
                    ...
                    <div key={i} className="px-5 py-3 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                      <div className="flex items-start gap-3">
                        <div className={`w-7 h-7 rounded flex items-center justify-center flex-shrink-0 ${
                          ...
                          log.action?.includes('CREATE') ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600' :
    ...
    <Layout>
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            {onlineOfficers.length} officier{onlineOfficers.length > 1 ? 's' : ''} en service • {patrols.length} patrouille{patrols.length > 1 ? 's' : ''}
          </p>
        </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-emerald-400 font-medium text-sm">
                En service depuis {formatDuration(serviceStatus.service?.current_duration)}
              </p>
              {serviceStatus.patrol && (
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className={`w-9 h-9 rounded flex items-center justify-center bg-${statusInfo.color}-50 dark:bg-${statusInfo.color}-900/20`}>
                          <statusInfo.icon size={18} className={`text-${statusInfo.color}-600`}/>
                        </div>
                        <div>
        ...
        <Modal title="Nouvel Appel" onClose={() => setShowDispatchModal(false)}>
          <form onSubmit={saveDispatch} className="space-y-4">
            ...
            <div className="flex gap-3">
              ...
              <Button type="submit" className="flex-1">Créer l'appel</Button>
            </div>
          </form>
        </Modal>
      )}
...
function Plaintes() {
  ...
  return (
    <Layout>
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Plaintes & Requêtes</h1>
...
function Admin() {
  ...
  const load = useCallback(() => {
    ...
    if(activeTab === 'logs' && hasPerm('view_logs')) {
      apiFetch("/api/admin/logs?limit=100").then(r => r?.ok ? r.json() : []).then(setLogs);
    }
    apiFetch("/api/admin/grades").then(r => r?.ok ? r.json() : []).then(setGrades);
  }, [activeTab, hasPerm]);
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                {logs.map((l, i) => (
                  <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                    ...
                    <td className="px-5 py-3">
                      <Badge variant={
                        l.action?.includes('DELETE') ? 'danger' :
                        l.action?.includes('CREATE') ? 'success' : 'default'
                      }>
                        {l.action}
                      </Badge>
                    </td>
