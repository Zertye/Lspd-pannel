...
router.delete("/users/:id", isAuthenticated, hasPermission('delete_users'), async (req, res) => {
  try {
    ...
    // Nettoyage des références - IMPORTANT: Mettre à jour les logs d'abord
    await pool.query("UPDATE logs SET user_id = NULL WHERE user_id = $1", [targetId]);
    ...
    // Nettoyage des tables Centrale
    await pool.query("DELETE FROM patrol_members WHERE user_id = $1", [targetId]);
    await pool.query("UPDATE centrale_operators SET is_active = FALSE WHERE user_id = $1", [targetId]);
    await pool.query("DELETE FROM centrale_operators WHERE user_id = $1", [targetId]);
    await pool.query("UPDATE centrale_notes SET author_id = NULL WHERE author_id = $1", [targetId]);
    await pool.query("UPDATE patrols SET created_by = NULL WHERE created_by = $1", [targetId]);
    await pool.query("UPDATE dispatch_calls SET dispatched_by = NULL WHERE dispatched_by = $1", [targetId]);
    await pool.query("DELETE FROM services WHERE user_id = $1", [targetId]);
